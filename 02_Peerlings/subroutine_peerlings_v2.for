...
SUBROUTINE UEXTERNALDB(LOP,LRESTART,TIME,DTIME,KSTEP,KINC)
	...
	IF (LOP.EQ.0) THEN		! START OF ANALYSIS
		...
		IO_STAT=-1
		OPEN(10,FILE=PATH//RUNTIME_FILE,STATUS='UNKNOWN',IOSTAT=IO_STAT)
		IF (IO_STAT.EQ.0) THEN
			...
			READ(10,*) C
			READ(10,*) KAPPA
			WRITE(6,'(A,I8)') 'FILE OPENED. IOSTAT=',IO_STAT
			WRITE(6,'(2(A,I8),7(A,F8.6))') 'NUMEL=',NUMEL,', NINTP=',NINTP,', CUTOFF=',CUTOFF,', ETA=',ETA,', NU=',NU,', ALPHA=',ALPHA,', BETA=',BETA,', C=',C,', KAPPA=',KAPPA
		ELSE
...
SUBROUTINE CHECK_JUMP(TIME1,TIME2,DTIME)
	...
	IF ((TIME1-NCYC_REAL).EQ.1) THEN
		...
		WRITE(6,'(/,A,F8.4,2(A,F8.6),3(A,I8))') 'AT ANALYSIS TIME(1)=',TIME1,', DDOT_MAX=',DDOT_MAX,', DMG_MAX=',DMG_MAX,', NOEL=',ITEMP,', NPT=',JTEMP,', NCYC_JUMP=',NCYC_JUMP
	...
	ELSE
		...
		IF (INCR_FLAG.EQ.1) THEN
			IO_STAT=-1
			INQUIRE(FILE=PATH//OUTPUT_FILE, EXIST=EXIST_FLAG)
			IF (EXIST_FLAG.AND.(NCYC_REAL.EQ.1)) THEN
				OPEN(30, FILE=PATH//OUTPUT_FILE, STATUS='OLD', ACTION='WRITE',IOSTAT=IO_STAT)
			ELSEIF (EXIST_FLAG.AND.(NCYC_REAL.NE.1)) THEN
				OPEN(30, FILE=PATH//OUTPUT_FILE, STATUS='OLD', POSITION='APPEND', ACTION='WRITE',IOSTAT=IO_STAT)
			ELSE
				OPEN(30, FILE=PATH//OUTPUT_FILE, STATUS='NEW', ACTION='WRITE',IOSTAT=IO_STAT)
			END IF
